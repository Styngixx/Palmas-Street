document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateSummary(breakdown) {
    if (!summaryItems) return; // Seguro anti-errores
  ...
  function renderCart() {
    ...
    breakdown.lines.forEach(({ item, unitPrice, finalLineTotal, bulkEligible, saleLabel }) => {
        ...
        <div class="card-body p-3">
          <div class="d-flex gap-3 align-items-center flex-wrap">
            ...
            <div class="flex-grow-1">
              ...
              <p class="text-muted mb-0 small">${PalmasCart.escapeHTML(item.marca || 'Palmas Street')} | Categoría: ${PalmasCart.escapeHTML(item.categoria || 'General')}</p>
              ${priceHTML}
            </div>
      ...
      card.querySelector('.btn-minus').addEventListener('click', () => {
        ...
        if (newQuantity <= 0) PalmasCart.removeFromCart(item.id);
        else PalmasCart.updateQuantity(item.id, newQuantity);
        renderCart();
      });
  ...
  // ===============================================
  // BOLETA BLINDADA (NUEVA VERSIÓN NUCLEAR)
  // ===============================================
  function finishPurchase(e) {
    if (e) e.preventDefault();

    // 1. Extraemos los datos ANTES de vaciar nada
    const cart = PalmasCart.getCart();
    if (!cart.length) {
    ...
    try {
      ...
      const rows = breakdown.lines.map(({ item, unitPrice, finalLineTotal }) => {
        return `
          <tr>
            ...
            <td style="padding: 12px 8px; border-bottom: 1px solid #eee; text-align: center; color: #2c3e50;">${Number(item.cantidad)}</td>
            <td style="padding: 12px 8px; border-bottom: 1px solid #eee; text-align: right; color: #2c3e50;">${PalmasCart.formatPrice(unitPrice)}</td>
            <td style="padding: 12px 8px; border-bottom: 1px solid #eee; text-align: right; font-weight: bold; color: #1a252f;">${PalmasCart.formatPrice(finalLineTotal)}</td>
          </tr>
        `;
      }).join('');
      ...
      if (breakdown.clearanceTotal > 0) {
        discountRows.push(`
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px; color: #dc3545; font-size: 0.95rem;">
            <span>Liquidaciones / remates:</span> <strong>- ${PalmasCart.formatPrice(breakdown.clearanceTotal)}</strong>
          </div>
        `);
      }
      if (breakdown.bulkTotal > 0) {
        discountRows.push(`
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px; color: #198754; font-size: 0.95rem;">
            <span>Descuento 3+ unidades (15%):</span> <strong>- ${PalmasCart.formatPrice(breakdown.bulkTotal)}</strong>
          </div>
        `);
      }
      if (breakdown.couponDiscount > 0 && breakdown.appliedCoupon) {
        discountRows.push(`
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px; color: #0d6efd; font-size: 0.95rem;">
            <span>Cupón ${breakdown.appliedCoupon.code}:</span> <strong>- ${PalmasCart.formatPrice(breakdown.couponDiscount)}</strong>
          </div>
        `);
      }
      ...
      const htmlBoleta = `
        <div class="receipt-box" style="background: #ffffff; padding: 40px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.08); max-width: 800px; margin: 0 auto; border: 1px solid #e0e0e0; font-family: Arial, sans-serif;">
          <div style="text-align: center; border-bottom: 2px dashed #cccccc; padding-bottom: 25px; margin-bottom: 25px;">
            <h2 style="font-weight: 900; margin-bottom: 5px; color: #1a252f; text-transform: uppercase; letter-spacing: 2px; font-size: 2rem;">Palmas Street</h2>
            <p style="color: #7f8c8d; margin-bottom: 20px; font-size: 0.95rem;">Frescura, atrevimiento y moda.</p>
            <div style="display: inline-block; background: #f8f9fa; padding: 12px 25px; border-radius: 8px; border: 1px solid #eeeeee;">
              <h4 style="margin: 0 0 5px 0; font-weight: 800; color: #2c3e50; font-size: 1.1rem;">BOLETA DE VENTA ELECTRÓNICA</h4>
              <p style="margin: 0; font-family: monospace; font-size: 1.2rem; color: #1a252f; letter-spacing: 1px;">${receiptNumber}</p>
            </div>
          </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <thead>
                <tr style="background-color: #f1f3f5;">
                  <th style="padding: 12px 8px; text-align: left; border-bottom: 2px solid #dee2e6; color: #495057; text-transform: uppercase; font-size: 0.85rem;">Producto</th>
                  <th style="padding: 12px 8px; text-align: center; border-bottom: 2px solid #dee2e6; color: #495057; text-transform: uppercase; font-size: 0.85rem;">Cant.</th>
                  <th style="padding: 12px 8px; text-align: right; border-bottom: 2px solid #dee2e6; color: #495057; text-transform: uppercase; font-size: 0.85rem;">Precio U.</th>
                  <th style="padding: 12px 8px; text-align: right; border-bottom: 2px solid #dee2e6; color: #495057; text-transform: uppercase; font-size: 0.85rem;">Subtotal</th>
                </tr>
              </thead>
              <tbody>
                ${rows}
          ...
          <div style="border-top: 2px solid #dee2e6; padding-top: 20px; width: 65%; margin-left: auto;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px; padding-top: 20px; border-top: 2px dashed #cccccc;">
              <span style="font-size: 1.3rem; font-weight: 900; color: #2c3e50;">TOTAL A PAGAR:</span>
              <span style="font-size: 1.8rem; font-weight: 900; color: #1a252f;">${PalmasCart.formatPrice(breakdown.finalTotal)}</span>
            </div>
          </div>
          <div style="text-align: center; margin-top: 40px; padding: 20px; background: #f8f9fa; border-radius: 8px; border-left: 5px solid #1a252f;">
            <strong style="color: #2c3e50; font-size: 1.1rem;">¡Gracias por tu compra en Palmas Street!</strong><br>
            <span style="color: #6c757d; font-size: 0.9rem;">Tu estilo, tus reglas. Vuelve pronto.</span>
          </div>
      ...
      // 3. INYECCIÓN NUCLEAR: Remplazamos todo el HTML de la página del carrito por la boleta
      // Así ningún código viejo puede ocultarla
      ...
      // Mandar el scroll para arriba para que se vea perfecta la factura
